...
<?if($arResult['ITEMS']):?>
	...
	<?if(!$arParams['IS_AJAX']):?>
		...
		<div class="items-list__gridForServices <?//=$gridClass?>">
	<?endif;?>
			<?
			$bShowImage = 
				$bIcons || 
...
<!-- $imageSrc - картинка URL
	<div class="items-list-inner__item-section<?=($bActiveDate ? ' bordered rounded-4 font_12 color_333' : ' font_13 color_999')?>"> категория
				...
				<div class="items-list-inner__wrapper <?=$itemWrapperClasses?> <?=$mixitup_class;?>" data-ref="mixitup-target">
					<div class="items-list-inner__item <?//=$itemClasses?><?=($bDetailLink ? '' : ' items-list-inner__item--cursor-initial')?><?=(!$bShowImage || !$imageSrc ? ' items-list-inner__item-without-image' : '')?><?=($bSale ? ' items-list-inner__item--sale' : '')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
						<?if($bShowImage && $imageSrc):?>
							<div class="items-list-inner__item-image-wrapper <?=$imageWrapperClasses?>" style="background-image: url(<?=$imageSrc?>">
<!--								--><?//if($bDetailLink):?>
<!--									<a class="items-list-inner__item-link" href="--><?php //=$detailUrl?><!--">-->
<!--								--><?//else:?>
<!--									<span class="items-list-inner__item-link">-->
<!--								--><?//endif;?>
<!--									--><?//if($bIcons && $nImageID):?>
<!--										--><?php //=CAllcorp3::showIconSvg(' fill-theme items-list-inner__item-image-icon', $imageSrc);?>
<!--									--><?//else:?>
<!--										<span class="items-list-inner__item-image --><?php //=$imageClasses?><!--" style="background-image: url(--><?php //=$imageSrc?><!--</span> -->
<!--									--><?//endif;?>
<!--								--><?//if($bDetailLink):?>
<!--									</a>-->
<!--								--><?//else:?>
									...
									<div class="items-list-inner__item-text-top-part <?=(!$arParams['NARROW'] && !($bShowImage && $imageSrc)) ? 'flex-1' : ''?>">
										...
										<?if(
											...
											<div class="items-list-inner__item-before-title line-block line-block--16 line-block--8-vertical line-block--flex-wrap<?=($bSections && $bActiveDate ? ' items-list-inner__item-before-title--has-bordered-section' : '')?>">
												<?// section title?>
												<?if($bSections):?>
													<div class="line-block.line-block__item__item">
										...
										<div class="items-list-inner__item-title switcher-title font_<?=$arParams['NAME_SIZE']?>">
											<?if($bDetailLink):?>
												<a class="items-list-inner__item-title-link" href="<?=$detailUrl?>"><?=$arItem['NAME']?></a>
											<?else:?>
												<span class="color_333"><?=$arItem['NAME']?></span>
											<?endif;?>
										</div>
										...
										<?if($bActiveDate):?>
											<div class="line-block__item">
												<div class="items-list-inner__item-period font_13 color_fff<?=($bSale && $arItem['ACTIVE_TO'] ? ' red' : '');?>">
													<?if($bSale):?>
