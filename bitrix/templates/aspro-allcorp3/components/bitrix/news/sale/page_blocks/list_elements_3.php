...
if($arParams['SECTION_ELEMENTS_TYPE_VIEW'] === 'FROM_MODULE'){
	$blockTemplateOptions = $GLOBALS['arTheme']['SALE_PAGE']['LIST'][$GLOBALS['arTheme']['SALE_PAGE']['VALUE']];
	$bItemsOffset = $blockTemplateOptions['ADDITIONAL_OPTIONS']['ELEMENTS_OFFSET_SALE']['VALUE'] === 'Y';
	$countElements = $blockTemplateOptions['ADDITIONAL_OPTIONS']['ELEMENTS_COUNT_SALE']['VALUE'];
	$bBottomImg = $blockTemplateOptions['ADDITIONAL_OPTIONS']['ELEMENTS_IMAGE_POSITION_SALE']['VALUE'] === 'BOTTOM';
}
else{
	$bItemsOffset = $arParams['ELEMENTS_ITEMS_OFFSET'] === 'Y';
	$countElements = $arParams['ELEMENTS_COUNT'];
	$bBottomImg = $arParams['ELEMENTS_IMAGE_POSITION'] === 'BOTTOM';
}
?>
<?$APPLICATION->IncludeComponent(
	...
	Array(
		...
		"DISCOUNT_POSITION" => $bBottomImg?"BOTTOM_CORNER":"TOP_CORNER",
		"NAME_SIZE" => "16",
		"MAXWIDTH_WRAP" => false,
		"COLUMN_REVERSE" => $bBottomImg,
		"NARROW" => true,
